...
raw_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", park, "&startMonth=", startMonth,"&startYear=", startYear, "&endMonth=", endMonth, "&endYear=", endYear))
...
```{r}
joined_park_percent <- joined_data %>%
  mutate(percent = (RecreationVisitors.x/RecreationVisitors.y * 100))

  
plotly::ggplotly(
  ggplot(data=joined_park_percent) + 
  geom_point(aes(x = Year, y = percent, color = UnitName.x)) +
  geom_path(aes(x = Year, y = percent, color = UnitName.x)) +
  ylab("Percent of Total Annual Park Visits") +
  ggtitle("Percent of Total Annual Park Visits by Park") +
  theme_bw(base_size = 10)
)
...
view(park_vistation_rank)
```

The Great Smokey Mountains National Park ranks the highest visited park with 9,907,814 visitors between 1990-2021. Acadia National Park ranks as the lowest visited park with 2,636,266 visitors between 1990-2021. 
