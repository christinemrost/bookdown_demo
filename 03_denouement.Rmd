...
co_water_data <- function(site, startw_year, endw_year)
  { raw_water_data <- GET(url = paste0("https://dwr.state.co.us/Rest/GET/api/v2/surfacewater/", "surfacewatertsday/?format=json&dateFormat=dateOnly&fields=", "abbrev%2CmeasDate%2Cvalue%2CmeasUnit&encoding=deflate&abbrev=", site, "&min-measDate=10%2F01%2F", startw_year - 1, "&max-measDate=09%2F30%2F", endw_year)) 
...
map_co_water_data<- sites %>%
  map(~co_water_data(., startw_year = 2021, endw_year = 2022))
...
combo <- bind_rows(mut_co_water_data, mut_usgs)
...
##105 days had a lower flow at our downstream locations. I do not have experience in watershed sciences and did not have any expectations while coding. I do think that 105 days is a relatively reasonable number as it is less than half the year. 
```
