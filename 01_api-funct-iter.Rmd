...
parkwide_visitation <- function(year = 1980){
...
#test <- parkwide_visitation(year = 1980)

# for loops
years <- c(1980, 1999, 2018)
output_triloop <- vector("list", length(years))
for(i in 1:length(years)) {
  output_triloop[[i]] <- parkwide_visitation(year = years[i])
} 
```
...
```{r}
packed_data <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021")
# convert content to text
unpacked_data <- httr::content(packed_data, as = "text", encoding = "UTF-8") 
final_data <- jsonlite::fromJSON(unpacked_data)
```
...
test1 <- parkwide_visitation(year = 1999)
view(test1)
...
packed_data2 <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/total/2021", park, "&startMonth=", startMonth,"&startYear=", startYear,"&endMonth=", endMonth, "&endYear=", endYear))
 
unpacked_data2 <- httr::content(packed_data, as = "text", encoding = "UTF-8")
  
final_data2 <- jsonlite::fromJSON(unpacked_data)
  return(final_data2)
}
...
```{r}
pull_ROMO <- unit_visitation(park = "ROMO", startMonth = 01, startYear = 1990, endMonth = 12, endYear = 2021)

pull_EVER <- unit_visitation(park = "EVER", startMonth = 01, startYear = 1990, endMonth = 12, endYear = 2021)

pull_THRO <- unit_visitation(park = "THRO", startMonth = 01, startYear = 1990, endMonth = 12, endYear = 2021)
```
...
```{r}
unit_visitation <- function(park, startYear=1980, endYear=2021){ 

  raw3<- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=" , park, "&startMonth=1&startYear=", startYear, "&endMonth=12&endYear=", endYear)) 
...
```{r}
#unit_visitation <- function(parks, startMonth = "01", startYear = "1980", endMonth = "12", endYear = "2021"){

#raw_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", parks, "&startMonth=", startMonth,"&startYear=", startYear, "&endMonth=", endMonth, "&endYear=", endYear))
...
```{r}
unit_visitation(park = "ROMO", startYear = 1980, endYear = 2021)
unit_visitation(park = "EVER", startYear = 1980, endYear = 2021)
unit_visitation(park = "THRO", startYear = 1980, endYear = 2021)
```
...
for(i in 1:length(parks)){
 output_floop[[i]] <- unit_visitation(park =parks[i]) 
}
...
output_map_parks <- parks %>%
  map(~ unit_visitation(park = .))
multi_map_years <- output_map_parks %>%
  bind_rows()
```
